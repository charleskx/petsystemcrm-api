...
export async function productsRoutes(app: FastifyInstance) {
	...
	app.get("/products/categories", { preHandler }, async (request, reply) => {
	...
	app.get("/products/alerts", { preHandler }, async (request, reply) => {
		const alerts = await getProductAlerts(request.tenantId)
		return reply.send(alerts)
	})

	app.get("/products", { preHandler }, async (request, reply) => {
	...
	app.get("/products/:id", { preHandler }, async (request, reply) => {
		const { id } = request.params as { id: string }
		try {
